...
<link rel="stylesheet" href="<?php echo get_template_directory_uri(); ?>/assets/css/admin.css">

<div class="admin-user-edit-area">
    <div class="admin-title">
        <?php echo $user_id ? "管理者編集" : "新規管理者作成"; ?>
    </div>

    <div class="admin-spirit-sheets-button-flex">
        <div class="admin-preview-button-flex">
            <div class="admin-preview-button-flex-box">
                <button class="admin-preview-button" type="button" onclick="location.href='<?php echo getURLSetSlag('admin-make-admin-user-list'); ?>'" style="background-color: #D6E9F5;">戻る</button>
            </div>
        </div>
    </div>
    ...
    <form method="post" class="admin-user-edit-form" onsubmit="return validateForm(this);">
        <div class="admin-user-edit-form-item">
            ...
            <input type="text" id="username" name="username" value="<?php echo $user ? esc_attr($user->user_login) : ''; ?>" required pattern="^[a-zA-Z0-9]+$" onkeyup="this.value=this.value.replace(/[^a-zA-Z0-9]/g,'')">
            <p class="admin-form-note">※半角英数字のみ使用できます</p>
        </div>
        ...
        <div class="admin-user-edit-form-submit">
            ...
            <?php if ($user_id) : ?>
                <button type="button" class="admin-preview-button" onclick="if(confirm('この管理者を削除してもよろしいですか？\n\n※この操作は取り消せません。')) location.href='<?php echo getURLSetSlag('admin-make-admin-user-list'); ?>?delete_user_id=<?php echo $user_id; ?>'" style="background-color: #ff4444; margin-left: 10px;">削除</button>
            <?php endif; ?>
        </div>
    </form>
</div>

<!-- WordPressメディアライブラリの読み込み -->
...
function validateForm(form) {
    // 確認ダイアログの表示
    if (!confirm('<?php echo $user_id ? 'このユーザー情報を更新してもよろしいですか？' : '新しい管理者を作成してもよろしいですか？'; ?>')) {
        return false;
    }

    // ユーザーネームのバリデーション
    const userLogin = form.username.value;
    if (!/^[a-zA-Z0-9]+$/.test(userLogin)) {
        alert('ユーザーネームは半角英数字のみ使用できます。');
        return false;
    }
