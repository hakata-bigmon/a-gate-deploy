...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($error_message)) {
        // 処理後は一覧ページにリダイレクト
        echo '<script>window.location.href = "' . getURLSetSlag('admin-make-teaching-user-list') . '";</script>';
        exit;
    }
}
...
<div class="admin-user-edit-area">
    <div class="admin-title">
        <?php echo $user_id ? "先生編集" : "新規先生作成"; ?>
    </div>

    <div class="admin-spirit-sheets-button-flex">
        <div class="admin-preview-button-flex">
            <div class="admin-preview-button-flex-box">
                <button class="admin-preview-button" type="button" onclick="location.href='<?php echo getURLSetSlag('admin-make-teaching-user-list'); ?>'" style="background-color: #D6E9F5;">戻る</button>
            </div>
        </div>
    </div>
    ...
    <form method="post" class="admin-user-edit-form" onsubmit="return validateForm(this);">
        ...
        <div class="admin-user-edit-form-submit">
            ...
            <?php if ($user_id) : ?>
                <button type="button" class="admin-preview-button" onclick="if(confirm('この先生を削除してもよろしいですか？\n\n※この操作は取り消せません。')) location.href='<?php echo getURLSetSlag('admin-make-teaching-user-list'); ?>?delete_user_id=<?php echo $user_id; ?>'" style="background-color: #ff4444; margin-left: 10px;">削除</button>
            <?php endif; ?>
        </div>
    </form>
</div>
...
function validateForm(form) {
    // 確認ダイアログの表示
    if (!confirm('<?php echo $user_id ? 'このユーザー情報を更新してもよろしいですか？' : '新しい先生を作成してもよろしいですか？'; ?>')) {
        return false;
    }
