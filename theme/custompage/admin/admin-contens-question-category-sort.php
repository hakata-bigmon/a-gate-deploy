<?php 
    ...
    if(isset($_POST["sorted_ids"])){
        ...
        foreach($sorted_ids as $key => $sorted_id){
            update_field("acf_question_category_sort", $key + 1, $sorted_id);
        }
    }


    $category_list = $spiritContensQuestion->getCategoryList(true);
      ...
      <table class="admin-table" id="question-table">
        <thead>
          <tr>
            ...
            <th data-sort="string">カテゴリー</th>
          </tr>
        </thead>
        <tbody id="sortable-tbody">
        <?php foreach($category_list as $key => $question_id){ ?>
          <tr data-id="<?php echo $question_id; ?>" data-index="<?php echo $key; ?>">
            <td class="sort-controls">
              ...
              <button type="button" class="sort-btn down" onclick="moveDown(<?php echo $key; ?>)" <?php echo ($key == count($category_list) - 1) ? 'disabled' : ''; ?>>▼</button>
            </td>
            <td class="order-number" style="text-align: center;font-weight: bold;"><?php echo $key; ?></td>
            <td class="original-number" style="text-align: center;"><?php echo get_field("acf_question_category_sort", $question_id) ?: ($key + 1); ?></td>
            <td><?php echo get_field("acf_question_category_name", $question_id); ?></td>
          </tr>
        <?php } ?>
        </tbody>
      </table>
...
function updateButtonStates() {
  ...
  // デバッグ用：ボタン状態をコンソールに出力
  console.log('Button states updated:', rows.length, 'rows');
  rows.forEach((row, idx) => {
    const upBtn = row.querySelector('.sort-btn.up');
    const downBtn = row.querySelector('.sort-btn.down');
    console.log(`Row ${idx}: Up=${upBtn?.disabled}, Down=${downBtn?.disabled}`);
  });
}
