<?php 
    ...
    if($schedule_data["人数"] - $schedule_data["予約人数"] - $schedule_people >= 0){
        ...
        for( $i=0; $i<$schedule_people; $i++)
        {
            ...
            //状況ステータスは入金待ちにする
            if($type_data["price"] > 0){
                $post_array["acf_purespirit_user_status"] = SpiritUserClass::MEMBER_STATUS_WATING_PAYMENT;
            }
            else{
...
<div class="user-top-area" style="margin-top: 40px;">
    ...
    <div class="user-jorei-section-title-wrap">
        <div class="user-jorei-section-title"><?php echo $saledata["表示名"];?>の予約枠確保</div>
    </div>
        ...
        <?php if($schedule_people_check){?>
            <div class="user-jorei-message-wrap">
                ...
                <?php if($type_data["price"] > 0){?>
                    ...
                    <br>
                    <br>
                    下記の決済画面より、お支払いをお願い致します。<br>
                    お支払いはTOP画面の詳細ページからも確認できます。<br>
                    <br>
                    <br>
                    <?php if($_POST["user-schedule-payment"] != SpiritUserClass::PAYMENT_TYPE_TRANSFER){ //振込以外?>
                        お支払いは、ご購入後１日以内にお願いします。<br>
                    <?php }else{?>
                        お支払いは、営業日３日以内にお願いします。<br>
                    <?php }?> 
                    期限を過ぎますとキャンセル扱いとなりますので<br>
                    ご注意ください。<br>
                

                     <div class="user-account-edit-form-btn-wrap" style="margin-top: 80px;margin-bottom: 400px;">
                        <a href="" class="user-account-edit-return-btn" style="width: 300px;height: 50px;margin-top: 100px;">決済画面へ  &gt;</a>
                    </div>
                <?php }else{?>
                    予約が完了しました。<br>
                    <br>
